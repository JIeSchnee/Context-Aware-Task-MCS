...
def parameters_initialisation(dict):
    ...
    for app in range(3):
        ...
        G, V, C, _, W = load_task(task_idx=app,
                                  dag_base_folder="/home/jiezou/Documents/Context_aware MCS/dag-gen-rnd-master/data/data-multi-m4-u0.7/4/")
        # print("G: ", G)
        # print("V: ", V)
        # print("C: ", C)
        # print("W: ", W)
...
def Tasks_degradation(dropped_task, model_pre, Tasks_pre, mgr_Task_set, keynode):
    ...
    model, Tasks = model_task_copy(model_pre, tasks_name_index, HI_group)
    dropped_task_set = []
    dropped_task_set.append(dropped_task)
    print("asfafwq", dropped_task_set)

    marginal_prob_set = Task_Dropping_Test(dropped_task_set, Tasks, model, keynode)
    for i in dropped_task_set:
        Tasks = remove_task(Tasks, i)

    mgr_Task_set.remove(dropped_task)

    return marginal_prob_set, model, Tasks, mgr_Task_set
...
if __name__ == "__main__":
    ...
    Appset_backpack = []
    Appset = Appset_original
    Appset_tt = copy.deepcopy(Appset)
